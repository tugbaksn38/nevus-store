// src/app/nevus/ev-tekstili/page.js
...
export default function TekstilPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch("/api/nevus/ev-tekstili");
    ...
    >
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white">ev-tekstili Ürünleri</h1>
          {!loading && !error && (
...
<AnimatedButton 
  href={`/nevus/ev-tekstili/${product.id}`} // ID'ye göre link
