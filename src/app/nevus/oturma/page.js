...
export default function OturmaPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch("/api/nevus/oturma");
    ...
    >
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Oturma Odası Ürünleri</h1>
          {!loading && !error && (
...
<AnimatedButton 
  href={`/nevus/oturma/${product.id}`} // ID'ye göre link
